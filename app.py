...
DAY_VOTE_SECONDS = 90          # cả làng có 90s để bỏ phiếu ban ngày
...
class GameRoom:
    ...
    # ------------------------------------------------------------------
    def enter_transition(self, seconds, message, pending_action, kind="pause"):
    ...
    def resolve_election(self):
        ...
        if chief:
            msg = f"{chief} đã được bầu làm Trưởng Làng! Phiếu của Trưởng Làng sẽ tính gấp đôi khi biểu quyết ban ngày."
        else:
            msg = "Làng không bầu được ai làm Trưởng Làng (phiếu không đủ đa số)."
        self.add_log(msg)
        self.last_election_result = {"chief": chief, "message": msg}
        self.election_votes = {}
        self.election_deadline = None
        self.enter_transition(REVEAL_SECONDS, msg, "after_election_reveal", kind="reveal")
    ...
    def _finish_night_actual(self):
        ...
        self.subphase = None
        self.subphase_deadline = None
        self.enter_transition(REVEAL_SECONDS, msg, "after_night_reveal", kind="reveal")

    def _open_day_or_end(self):
        if self.check_win():
            return
        self.phase = "day"
        self.votes = {}
    ...
    def _finish_day_actual(self):
        ...
        self.votes = {}
        self.day_deadline = None
        self.enter_transition(REVEAL_SECONDS, msg, "after_day_reveal", kind="reveal")
...
def wolf_action():
    ...
    room.add_log("Ma Sói đã chọn xong mục tiêu.")
